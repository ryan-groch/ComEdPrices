...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const setError = () => {
    fiveMinutesElem.textContent =
      prevHourElem.textContent =
      currHourElem.textContent =
        "Error";
  };

  const fetchData = async () => {
    const client = await getClient();
    const currHourFetch = schema.safeParse(
      (await client.get(currHourUrl)).data
    );
    const fiveMinuteFetch = schema.safeParse(
      (await client.get(fiveMinutesUrl)).data
    );
    ...
    for (const snapshot of fiveMinuteFetch.data) {
      ...
      if (!inCurrOrPrevHour(lastTimestamp, indexDate)) break;
